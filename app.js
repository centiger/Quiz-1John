...
function renderQuestion(){
  const q=quiz[pos], ref=`요일 ${q.chapter}:${q.verse}`, saved=answers[q.id] ?? '';
...
function feedbackHtml(q){
  ...
  return `<div class="feedback ${ok?'good':'bad'}"><b>${ok?'정답/핵심어 충족':'정답 확인'}</b><br>${detail}<br><span class="muted">성경 장절: 요일 ${q.chapter}:${q.verse}</span></div>`;
}
...
function showResult(){
  ...
  $('#reviewBtn').onclick=()=>{
    $('#reviewArea').innerHTML=quiz.map(q=>`<div class="bankitem"><h3>${q.id} · ${q.subtype==='빈칸형'?'빈칸형':q.type} · 난이도 ${q.difficulty} · 요일 ${q.chapter}:${q.verse}</h3><div><b>문제:</b> ${q.question}</div><div><b>내 답:</b> ${q.type==='객관식' && answers[q.id]!==undefined ? `${Number(answers[q.id])+1}. ${q.choices[answers[q.id]]}` : (answers[q.id]||'무응답')}</div><div><b>정답/예시:</b> ${q.answerText}</div></div>`).join('');
  };
}
$('#showAllBtn').addEventListener('click',()=>{
  $('#bankPanel').classList.remove('hidden');
  $('#bankList').innerHTML=QUESTION_BANK.map(q=>`<div class="bankitem"><h3>${q.id} · ${q.subtype==='빈칸형'?'빈칸형':q.type}${q.type==='주관식'&&q.subtype!=='빈칸형'?`(${q.subtype})`:''} · 난이도 ${q.difficulty} · 요일 ${q.chapter}:${q.verse}</h3><div><b>문제:</b> ${q.question}</div>${q.type==='객관식'?`<ol>${q.choices.map(c=>`<li>${c}</li>`).join('')}</ol>`:''}<details><summary>정답 보기</summary><p>${q.type==='객관식'?`${Number(q.answer)+1}번 · `:''}${q.answerText}</p></details></div>`).join('');
...
function answerTxt(){
  return QUESTION_BANK.map((q,i)=>`${i+1}. ${q.type==='객관식'?`${Number(q.answer)+1}번 `:''}${q.answerText} (요일 ${q.chapter}:${q.verse})${q.rubric?`\n   채점기준: ${q.rubric}`:''}`).join('\n');
}
document.querySelectorAll('[data-download]').forEach(btn=>btn.addEventListener('click',()=>{
  const kind=btn.dataset.download;
  if(kind==='questions-txt') download('요한일서_성경퀴즈_60문항_문제지.txt',questionTxt());
  if(kind==='answers-txt') download('요한일서_성경퀴즈_60문항_정답지.txt',answerTxt());
  if(kind==='json') download('요한일서_문제은행_120.json',JSON.stringify(QUESTION_BANK,null,2),'application/json;charset=utf-8');
  if(kind.includes('csv')){
    ...
    QUESTION_BANK.forEach((q,i)=>rows.push(answersMode?
      [i+1,q.type,q.subtype,q.difficulty,`요일 ${q.chapter}:${q.verse}`,q.type==='객관식'?`${Number(q.answer)+1}번 ${q.answerText}`:q.answerText,q.rubric||'']:
      [i+1,q.type,q.subtype,q.difficulty,`요일 ${q.chapter}:${q.verse}`,q.question,q.choices.join(' | ')]
    ));
    download(answersMode?'요한일서_성경퀴즈_정답지.csv':'요한일서_성경퀴즈_문제지.csv',rows.map(r=>r.map(csvEscape).join(',')).join('\n'),'text/csv;charset=utf-8');
  }
}));
